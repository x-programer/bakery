...
function Page() {
  ...
  useEffect(() => {
    const fetchCakes = async () => {
      try {
        const response = await fetch('/api/products?category=pastries');
    ...
    <>
      <div className='flex flex-col items-center justify-center sm:pt-[15rem] pt-[9rem]'>
        <div className='flex flex-col sm:flex-row'>
          <h1 className={`text-4xl uppercase ${raleway.className} text-gray-700 mx-4`}>Pastries - Pricing 200 ₹</h1>
          <p className='m-4 border border-black p-3 text-gray-700 bg-inherit'>
          One of our most diverse sections in our bakery are the pastries and they can speak for themselves on how good they are. If you’re a cake lover but aren’t looking for an entire cake we offer a smaller slice for you to take home and enjoy. Many of our slices and treats rotate so it’s always a good idea to contact us before heading over to the bakery for a visit. 
          </p>
        </div>
