...
function Page() {
  ...
  useEffect(() => {
    const fetchCakes = async () => {
      try {
        const response = await fetch('/api/products?category=macrons');
    ...
    <>
      <div className='flex flex-col items-center justify-center sm:pt-[15rem] pt-[9rem]'>
        <div className='flex flex-col sm:flex-row'>
          <h1 className={`text-4xl uppercase ${raleway.className} text-gray-700 mx-4`}>Macrons - Pricing 200 ₹</h1>
          <p className='m-4 border border-black p-3 text-gray-700 bg-inherit'>
          Bulk macaroon orders are available, and we even provide discounts based on the number you purchase. If you have a certain theme for your event and need colors and flavors to be mixed we can do that! Want a raspberry macaroon to look like a blueberry flavor, we can do that! Ask us about any other colors that and flavors that you may not see, our bakers are very talented and we can make anything work. Place your order with us today, and we will mix and match to your heart's content.
          </p>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {cakes.length > 0 ? (
              ...
              <p className='text-center'>Loading macrons...</p>
            )}
          </div>
        </div>
        {/* Cake cards end */}
